...
Перед первым запуском тестов (или если база данных тестов отсутствует), выполните скрипт инициализации из корня проекта:

```bash
./bin/setup-test-db
```

Этот скрипт автоматически создаст базу данных `symfony_db_test` и применит все необходимые миграции.
